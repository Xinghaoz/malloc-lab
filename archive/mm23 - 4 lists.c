...
/* This line is exceed 80 column, but I think it would be OK */
#define PREV_ADDR(bp) (IS_PREV_8_BYTE(bp) ? ((char *)bp - DSIZE): ((char *)(bp) - (GET(((char *)(bp) - DSIZE)) & ~(0x7))))
...
static void *size_24_list;  /* Pointer to 24 bytes free list */
static void *size_32_list;  /* Pointer to 24 bytes free list */
static void *size_40_list;  /* Pointer to 24 bytes free list */
int current_chuck = 1 << 8;
...
int mm_init(void)
{
	...
	size_24_list = MM_NULL;
	size_32_list = MM_NULL;
	size_40_list = MM_NULL;
...
void mm_checkheap(int version) {
	void *bp = heap_listp;
	/* If there is a bug in the free bit, 
	 * print the current whole heap and stop 
	 */
	/* Print the heap information - use for malloc */
	if (version == 1) {
		while (THIS_SIZE(bp) != 0) {			
			if(IS_ALLOC(bp) == 0 && (IS_ALLOC(PREV_ADDR(bp)) == 0 ||
					IS_ALLOC(NEXT_ADDR(bp)) == 0)) {
				print_heap(1);
				exit(0);
			}
			bp = NEXT_ADDR(bp);
	...
	/* Print the information of the 8-byte free list */
	} else if (version == 4) {
		while (THIS_SIZE(bp) != 0) {			
			if (IS_ALLOC(bp) == 0 && (IS_ALLOC(PREV_ADDR(bp)) == 0 ||
					IS_ALLOC(NEXT_ADDR(bp)) == 0)) {
	...
	/* Print the information of the 16-byte free list */
	} else if (version == 5) {
		while (THIS_SIZE(bp) != 0) {			
			if (IS_ALLOC(bp) == 0 && (IS_ALLOC(PREV_ADDR(bp)) == 0 ||
					IS_ALLOC(NEXT_ADDR(bp)) == 0)) {
...
static void *find_fit(size_t size)
{
    ...
    if (size <= 8 && size_8_list != MM_NULL) {
		...
		return size_16_list;
	} else if(size <= 24 && size_24_list != MM_NULL) {
		return size_24_list;
	} else if(size <= 32 && size_32_list != MM_NULL) {
		return size_32_list;
	} else if(size <= 40 && size_40_list != MM_NULL) {
		return size_40_list;
	}
	/* ///////// To be optimized ///////// */
...
inline static void insert_node(void *bp)
{
	...
	if (insert_size == 24) {
		RIGHT_PTR(bp, size_24_list);
		LEFT_PTR(size_24_list, bp);
        size_24_list = bp;
		LEFT_PTR(bp, MM_NULL);
		#ifdef DEBUG
		mm_checkheap(5);	
	...
	if (insert_size == 32) {
		RIGHT_PTR(bp, size_32_list);
		LEFT_PTR(size_32_list, bp);
        size_32_list = bp;
		LEFT_PTR(bp, MM_NULL);
		#ifdef DEBUG
		mm_checkheap(5);	
	...
	if (insert_size == 40) {
		RIGHT_PTR(bp, size_40_list);
		LEFT_PTR(size_40_list, bp);
        size_40_list = bp;
		LEFT_PTR(bp, MM_NULL);
		#ifdef DEBUG
		mm_checkheap(5);	
...
void print_heap(int version)
{
	...
	if (1 == version) {
		...
		while (1) {		
			...
			long this_addr = (long)(PREV_ADDR(bp));
			long prev_addr = (long)(NEXT_ADDR(bp));
			long next_addr = (long)mem_heap_hi();
		...
		while (THIS_SIZE(bp) != 0) {
			if (IS_ALLOC(bp) == 0 && (IS_ALLOC(PREV_ADDR(bp)) == 0 ||
				...
				if ((long)(bp) != (long)(PREV_ADDR(bp))) {
					...
					printf("this address: %lx  ", this_addr);
					printf("prev address: %lx  ", prev_addr);
					printf("next address: %lx  ", next_addr);
...
void print_list_8() {
	...
	while (MM_NULL != bp) {
		int is_this_alloc = IS_ALLOC(bp);
		int is_prev_alloc = IS_ALLOC(PREV_ADDR(bp));
		int is_next_alloc = IS_ALLOC(NEXT_ADDR(bp));

		long this_addr = (long)(bp);
		long prev_addr = (long)(PREV_ADDR(bp));
		long next_addr = (long)(NEXT_ADDR(bp));

		printf("is_this_alloc: %d  is_prev_alloc: %d  is_next_alloc: %d\n", 
				is_this_alloc, is_prev_alloc, is_next_alloc);
		printf("this address: %lx  ", this_addr);
		printf("prev address: %lx  ", prev_addr);
		printf("next address: %lx  ", next_addr);
		bp = GET_LEFT(bp);
