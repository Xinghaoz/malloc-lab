...
{	
	...
	if ( prev_alloc && next_alloc ){ /* Case 1 */
	    ...
	    size_t sign = 0 | IS_PREV_ALLOC(prev) | IS_PREV_FREE(prev);
	    delete_node(prev);
		PUT(HEADER_ADDR(prev), PACK(size, sign));
		PUT(FOOTER_ADDR(prev), PACK(size, sign));
		return prev;
	}
...
static void place(void *bp,size_t asize)
{
	...
	delete_node(bp);
	if((block_size - asize) >= OVERHEAD) {
	    size_t sign = 1 | IS_PREV_ALLOC(bp) | IS_PREV_FREE(bp);
		PUT(HEADER_ADDR(bp), PACK(asize,sign) );

		void * temp = NEXT_ADDR(bp);
		PUT(HEADER_ADDR(temp), PACK(block_size - asize,2) );
		PUT(FOOTER_ADDR(temp), PACK(block_size - asize,2) );
	    ...
	    size_t sign = 1 | IS_PREV_ALLOC(bp) | IS_PREV_FREE(bp);
		PUT(HEADER_ADDR(bp), PACK(block_size, sign));
	}
//	delete_node(bp);
}
...
{
	/* ///////// To be optimized ///////// */	
    if (size <= 8 && MM_NULL != mini_list_ptr) {
		return mini_list_ptr;
	} else if(size <= 16 && MM_NULL != small_list_ptr) {
...
inline static void delete_node(void *bp)
{
    ...
    if(bpsize < MIN_SIZE) return;
    /* Case that the block is one of the following ones in the node. */
    if( ((void *)GET_LEFT(bp) != MM_NULL) && ((void *)GET_BROTHER(GET_LEFT(bp) ) == bp)){
	...
	else if( (void *)GET_BROTHER(bp) == MM_NULL){
		if( bp == root ){/* the node is the root */
			if( (void *)GET_RIGHT(bp) == MM_NULL){/* no right child */
				...
				while( (void *)GET_LEFT(temp) != MM_NULL)
                    temp = (void *)GET_LEFT(temp);
...
void print_heap(int version)
{
	...
	if (1 == version) {
		...
		while(THIS_SIZE(bp) != 0){			
			printf("address bias:  %d / %d%d%d\n",(int)(bp - HEAP_BUTTOM), (int)IS_PREV_FREE(bp), (int)IS_PREV_ALLOC(bp), (int)IS_ALLOC(bp));                     
			printf("size        :  %d\n",(int)THIS_SIZE(bp));
			printf("next	    :  %d / %d\n",(int)THIS_SIZE(bp) + (int)(bp - HEAP_BUTTOM), (int)mem_heapsize());
			printf(" /// \n");		
			int aa = IS_ALLOC(bp);
